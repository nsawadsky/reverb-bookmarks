...
public class HistoryClassifier {
    ...
    public void testClassifier(String url) {
        ...
        case CODE_RELATED: {
            if (location.isJavadoc) {
                log.info("Classified as code-related, javadoc");
            } else {
                log.info("Classified as code-related, non-javadoc");
            }
            break;
        }
    ...
    public ClassifierData getResults() {
        ...
        List<LocationAndVisits> javadocLocations = new ArrayList<LocationAndVisits>();
        ...
        for (LocationAndVisits locationAndVisits: locationAndVisitsList) {
            if (locationAndVisits.location.locationType == LocationType.CODE_RELATED) {
                ...
                if (locationAndVisits.location.isJavadoc) {
                    javadocLocations.add(locationAndVisits);
                }
        ...
        return new ClassifierData(initialVisitCount, redirectVisitCount, overallRevisitRate,
                ...
                codeRelatedLocations.size(), javadocLocations.size(), nonCodeRelatedLocations.size(), unknownLocations.size(), 
    ...
    protected void classifyLocation(LocationAndVisits locationAndVisits, boolean dumpFile, IndexerConnection indexerConnection) {
        ...
        try {
            ...
            LocationInfo locationInfo = classifyDocument(doc, dumpFile);
            location.locationType = locationInfo.type;
    ...
    protected LocationInfo classifyDocument(Document doc, boolean dumpFile) {
        ...
        */
        ...
        if (matches.size() >= CODE_RELATED_MATCH_THRESHOLD) {
            return new LocationInfo(LocationType.CODE_RELATED, isJavadoc);
        } else {
            return new LocationInfo(LocationType.NON_CODE_RELATED, false);
        }
    }
    ...
    private List<HistoryVisit> filterRedirectsChrome(List<HistoryVisit> visitList) {
        List<HistoryVisit> result= new ArrayList<HistoryVisit>();
        for (HistoryVisit visit: visitList) {
            // From page_transition_types.h in Chrome source code.
    ...
    protected class LocationInfo {
        public LocationInfo(LocationType type, boolean isJavadoc) {
            this.type = type;
